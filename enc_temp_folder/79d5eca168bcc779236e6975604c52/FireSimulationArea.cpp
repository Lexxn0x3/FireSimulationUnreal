...
{
    const int32 XCount = FMath::CeilToInt(GridSize.X / VoxelSize);
    const int32 YCount = FMath::CeilToInt(GridSize.Y / VoxelSize);
    const int32 ZCount = FMath::CeilToInt(GridSize.Z / VoxelSize);
...
TArray<UFireVoxel2*> AFireSimulationArea::CalculateVoxelProperties(TArray<UFireVoxel2*> voxels)
{
    TMap<AActor*, float> Occupancy =  CalculateVoxelOccupancy(voxels);
    TArray<UFireVoxel2*> returnVoxels = GenerateVoxelsWithOccupancyProperties(Occupancy);

    return returnVoxels;
}

TArray<UFireVoxel2*> AFireSimulationArea::GenerateVoxelsWithOccupancyProperties(TMap<AActor*, float> Occupancy)
{
    TArray<UFireVoxel2*> returnVoxels;
...
TMap<AActor*, float> AFireSimulationArea::CalculateVoxelOccupancy(TArray<UFireVoxel2*> voxels)
{
    TMap<AActor*, float> occupancy;

    for (UFireVoxel2* Voxel : voxels)
    {
        FVector VoxelLocation = GetOffsetVoxel(Voxel->Position);

        TArray<FOverlapResult> OverlapResults;
        FCollisionQueryParams CollisionQueryParams;
        FCollisionObjectQueryParams ObjectQueryParams;

        bool bHit = GetWorld()->OverlapMultiByObjectType(OverlapResults, VoxelLocation, FQuat::Identity, ObjectQueryParams, FCollisionShape::MakeBox(FVector(VoxelSize / 2.f)));
        ...
        if (bHit)
        {
            ...
            {
                //Generate RandomPoints within Voxel
                FVector RandomPoint = FMath::RandPointInBox(FBox(VoxelLocation - FVector(VoxelSize / 2.f), VoxelLocation + FVector(VoxelSize / 2)));
                RandomPoints.Add(RandomPoint);
            }

            TMap<AActor*, int32> timesHit;

            for (FVector& RandomPoint : RandomPoints)
            {
                TArray<FHitResult> HitResults;
                FCollisionQueryParams CollisionQueryParamsPoint;
                GetWorld()->SweepMultiByChannel(HitResults, RandomPoint, RandomPoint + FVector(0.1f), FQuat::Identity, ECC_Visibility, FCollisionShape::MakeSphere(0.005), CollisionQueryParamsPoint);
                ...
                for (auto& HitResult : HitResults)
                {
                    ...
                    if (HitActor)
                    {
                        // If the actor is already in the map, increment its count; otherwise, add it to the map with a count of 1
                        timesHit.FindOrAdd(HitActor)++;
                    }
                }
            }
            ...
            for (auto& Entry : timesHit)
            {
                occupancy.Add(Entry.Key, static_cast<float>(Entry.Value) / TotalPoints);
            }
...
void AFireSimulationArea::DrawDebugOverlay()
{
    ...
    for (const FVoxelDistance& VoxelDistance : ClosestVoxels)
    {
        ...
        DrawDebugBox(GetWorld(), CubePosition, FVector(VoxelSize) / 2.f, GetHeatMapColorForVoxel(Voxel->Temperature), false, 0.f, 0, 2.f);
...
FVector AFireSimulationArea::GetOffsetVoxel(FVector CubePosition)
{
    ...
    return FVector(CubePosition.X * VoxelSize + VoxelSize / 2 + Origin.X, CubePosition.Y * VoxelSize + VoxelSize / 2 + Origin.Y, CubePosition.Z * VoxelSize + VoxelSize / 2 + Origin.Z);
}
